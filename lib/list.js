async function get(ctx, type) {
    try {
        ...
        switch (type) {
            ...
            case "osettext": {
                const data = [
                    "donate - Variabel tersedia: %tag%, %name%, %prefix%, %command%, %footer%, %readmore% (Atur teks donasi)",
                    "price - Variabel tersedia: %tag%, %name%, %prefix%, %command%, %footer%, %readmore% (Atur teks harga)",
                    "qris (Atur gambar QRIS untuk donasi, harus berupa link)"
                ];
                text = createList(data, (list) => `❖ ${list}`);
                break;
            }

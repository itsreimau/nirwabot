...
class Ctx {
    ...
    get quoted() {
        ...
        const senderJid = context.participant || chat;
        const fakeMsg = {
            key: {
                remoteJid: chat,
                id: context.stanzaId,
                fromMe: Baileys.areJidsSameUser(senderJid, this.me?.id),
                participant: Baileys.isJidGroup(chat) ? senderJid : null
            },
            message: raw,
            body: helper.getBodyFromMsg({
                message: raw
            }),
            pushName: helper.getPushName(senderJid, this._db)
        };
        return new Ctx({
            used: this._used,
            args: this._args,
